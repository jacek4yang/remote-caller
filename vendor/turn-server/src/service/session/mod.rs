...
/// Unauthenticated challenge sessions are deliberately short lived. A public
/// UDP endpoint can otherwise retain spoofed source tuples for ten minutes.
pub const NEW_SESSION_LIFETIME: u64 = 30;

/// Default authenticated allocation lifetime in seconds (10 minutes).
pub const DEFAULT_SESSION_LIFETIME: u64 = 600;

/// Longest allocation lifetime accepted from a client.
pub const MAX_SESSION_LIFETIME: u64 = 3_600;
    ...
    T: ServiceHandler,
{
    pub fn new(options: SessionManagerOptions<T>) -> Arc<Self> {
        let derived_limit = options.port_range.size().saturating_mul(4).max(64);
        Self::new_bounded(options, derived_limit)
    }

    pub fn new_bounded(options: SessionManagerOptions<T>, max_sessions: usize) -> Arc<Self> {
        let max_sessions = max_sessions.max(1);
        let port_capacity = options.port_range.size();
        let this = Arc::new(Self {
            port_allocator: Mutex::new(PortAllocator::new(options.port_range)),
            port_mapping_table: RwLock::new(Table::with_capacity(port_capacity)),
            sessions: RwLock::new(Table::with_capacity(max_sessions)),
            timer: Timer::default(),
                ...
                sessions.insert(
                    *key,
                    Session::New {
                        ...
                        expires: self.timer.get() + NEW_SESSION_LIFETIME,
                    },
                );
            }
        }
    ...
    pub async fn get_password(
        ...
        // Already authenticated, get the cached digest directly.
        {
            ...
            if let Some(Session::Authenticated { password, .. }) = sessions.get(identifier) {
                return Some(*password);
            }
            if !sessions.contains_key(identifier) && sessions.len() >= self.max_sessions {
                return None;
            }
        }
        ...
        // Record a new session.
        {
            ...
            if !lock.contains_key(identifier) && lock.len() >= self.max_sessions {
                return None;
            }
